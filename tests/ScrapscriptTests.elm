    ...
    desc "Scrapscript"
        ...
        , desc "Eval" <|
            List.map (\( env, k, v ) -> test k <| always <| expectEqual v <| run env k)
                ...
                , ( Dict.empty, "1 + \"hello\"", Err "expected Int or Float, got Text" )
                ...
                , ( Dict.empty, "123 ++ \" world\"", Err "expected Text, got Int" )
                , ( Dict.empty, "\" world\" ++ 123", Err "expected Text, got Int" )
                ...
                , ( Dict.empty, "[4] @ \"hello\"", Err "cannot index into list using type Text, expected integer" )
                ...
                , ( Dict.empty, "\"xyz\" < 4", Err "expected Int or Float, got Text" )
                , ( Dict.empty, "3 <= 4", Ok true )
                , ( Dict.empty, "\"xyz\" <= 4", Err "expected Int or Float, got Text" )
   ...
   suite =
       desc "Scrapscript"
           ...
           , desc "match"
               ...
               , test "int_with_non_int_returns_none" <| \_ -> expectEqual ( match ( Text "abc", Int 1 ), Err "" )
               ...
               , test "float_with_non_float_raises_match_error" <| \_ -> expectError "pattern matching is not supported for Floats" <| match ( Text "abc", Float 1 )
               , test "with_equal_strings_returns_empty_dict" <| \_ -> expectEqual ( match ( Text "a", Text "a" ), Ok [] )
               , test "with_inequal_strings_returns_none" <| \_ -> expectEqual ( match ( Text "b", Text "a" ), Err "" )
               , test "string_with_non_string_returns_none" <| \_ -> expectEqual ( match ( Int 1, Text "abc" ), Err "" )
               , test "var_returns_dict_with_var_name" <| \_ -> expectEqual ( match ( Text "abc", Var "a" ), Ok [ ( "a", Text "abc" ) ] )
